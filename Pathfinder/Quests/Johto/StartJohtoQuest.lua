...
function StartJohtoQuest:NewBarkTown()
	if getTeamSize() == 0 then
		return PathFinder.moveTo(getMapName(), "Professor Elms Lab")
	else
...
function StartJohtoQuest:NewBarkTownPlayerHouseBedroom()
	return PathFinder.moveTo(getMapName(), "New Bark Town Player House")
end
...
function StartJohtoQuest:Route29()
	if isNpcOnCell(23,11) then --Item: Berry Tree
		...
		return PathFinder.moveTo(getMapName(), "Cherrygrove City")
	end	
end
...
function StartJohtoQuest:CherrygroveCity()
	...
	elseif self:needPokecenter() or not Game.isTeamFullyHealed() or not self.registeredPokecenter == "Pokecenter Cherrygrove City" then
		return PathFinder.moveTo(getMapName(), "Pokecenter Cherrygrove City")
	elseif self:needPokemart() then
		return PathFinder.moveTo(getMapName(), "Mart Cherrygrove City")
	else
...
function StartJohtoQuest:Route30()
	...
	elseif self.BUY_BIKE and getMoney() > 75000 and not hasItem("Bicycle") and not hasItem("Bike Voucher") then
		return PathFinder.moveTo(getMapName(), "Route 30 House 2")
	elseif Game.tryTeachMove("Cut","HM01 - Cut") == true then
		return PathFinder.moveTo(getMapName(), "Route 31")
	end
end
...
function StartJohtoQuest:Route30House2()
	if self.BUY_BIKE and getMoney() > 75000 and not hasItem("Bicycle") and not hasItem("Bike Voucher") then
		...
		return PathFinder.moveTo(getMapName(), "Route 30")
	end
end
...
function StartJohtoQuest:Route31()
	...
	elseif isNpcOnCell(24,15) then
		...
		return PathFinder.moveTo(getMapName(), "Violet City Stop House")
	end
end

function StartJohtoQuest:VioletCityStopHouse()
	return PathFinder.moveTo(getMapName(), "Violet City")
end

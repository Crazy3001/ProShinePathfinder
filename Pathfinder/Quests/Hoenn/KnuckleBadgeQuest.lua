...
function KnuckleBadgeQuest:Route104()
	if Game.inRectangle(7,0,41,67) then 
		...
		else PathFinder.moveTo(getMapName(), "Petalburg Woods")
		end
	elseif not hasItem("Knuckle Badge") then
	PathFinder.moveTo(getMapName(), "Route 104 Sailor House") 
...
function KnuckleBadgeQuest:DewfordTown()
	if self:needPokecenter() or not Game.isTeamFullyHealed() or self.registeredPokecenter ~= "Pokecenter Dewford Town" then
		return PathFinder.moveTo(getMapName(), "Pokecenter Dewford Town")
	elseif hasItem("SLetter") then 
		PathFinder.moveTo(getMapName(), "Route 106")
	elseif not self:isTrainingOver() then
		PathFinder.moveTo(getMapName(), "Route 106")
	elseif not hasItem("Knuckle Badge") then
		if isNpcOnCell(22,23)then 
			talkToNpcOnCell(22,23)
		else PathFinder.moveTo(getMapName(), "Dewford Town Gym")
		end
...
function KnuckleBadgeQuest:GraniteCave1F()
	...
	else PathFinder.moveTo(getMapName(), "Route 106")
	end
end
...
function KnuckleBadgeQuest:GraniteCave1F2()
	if isNpcVisible("Steven") then 
		talkToNpc("Steven")
	else PathFinder.moveTo(getMapName(), "Granite Cave 1F")
	end
end
...
function KnuckleBadgeQuest:DewfordTownGym()
	...
	else PathFinder.moveTo(getMapName(), "Dewford Town")
	end
end

function KnuckleBadgeQuest:Route106()
	if self:needPokecenter() then
		PathFinder.moveTo(getMapName(), "Dewford Town")
	elseif hasItem("SLetter") or not self:isTrainingOver()then 
		PathFinder.moveTo(getMapName(), "Granite Cave 1F")
	else PathFinder.moveTo(getMapName(), "Dewford Town")
	end
end
...
function KnuckleBadgeQuest:RustboroCityGym()
	PathFinder.moveTo(getMapName(), "Rustbor City")
end	


function KnuckleBadgeQuest:DevonCorporation1F()
	if not dialogs.Steven.state then
		PathFinder.moveTo(getMapName(), "Devon Corporation 2F")
	else PathFinder.moveTo(getMapName(), "Rustboro City")
end
end

function KnuckleBadgeQuest:DevonCorporation2F()
	if not dialogs.Steven.state then
		PathFinder.moveTo(getMapName(), "Devon Corporation 3F")
	else PathFinder.moveTo(getMapName(), "Devon Corporation 1F")
end
end

function KnuckleBadgeQuest:RustboroCity()
	PathFinder.moveTo(getMapName(), "Devon Corporation 1F")
end
...
function KnuckleBadgeQuest:DevonCorporation3F()
	if not dialogs.Steven.state then
		talkToNpc("Mr. Stone")
	else PathFinder.moveTo(getMapName(), "Devon Corporation 2F")
end
end

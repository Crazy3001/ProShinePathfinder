...
function ThunderBadgeQuest:VermilionCity()
	...
	if self:needPokecenter() or not Game.isTeamFullyHealed() or not self.registeredPokecenter == "Pokecenter Vermilion" then
		return PathFinder.moveTo(getMapName(), "Pokecenter Vermilion")
	...
	elseif not self:isTrainingOver() then
		return PathFinder.moveTo(getMapName(), "Route 6")-- Go to Route 6 and Leveling
	...
	elseif not hasItem("Thunder Badge") then
		if Game.tryTeachMove("Cut","HM01 - Cut") == true then
			return PathFinder.moveTo(getMapName(), "Vermilion Gym")
		end
	else
...
function ThunderBadgeQuest:VermilionGym()
	...
	elseif not self:isTrainingOver() and not hasItem("Thunder Badge") then
		return PathFinder.moveTo(getMapName(), "Vermilion City")-- Go to Route 6 and Leveling
	else

...
function HmSurfQuest:SafariEntrance()
	if not hasItem("HM03 - Surf") then
		return PathFinder.moveTo(getMapName(), "Safari Area 1")
	else
...
function HmSurfQuest:SafariArea1()
	if not hasItem("HM03 - Surf") then
		return PathFinder.moveTo(getMapName(), "Safari Area 2")
	else
		return PathFinder.moveTo(getMapName(), "Safari Entrance")
	end
end

function HmSurfQuest:SafariArea2()
	if not hasItem("HM03 - Surf") then
		return PathFinder.moveTo(getMapName(), "Safari Area 3")
	else
		return PathFinder.moveTo(getMapName(), "Safari Area 1")
	end
end
...
function HmSurfQuest:SafariArea3()
	if not hasItem("HM03 - Surf") then
		return PathFinder.moveTo(getMapName(), "Safari House 4")
	else
		return PathFinder.moveTo(getMapName(), "Safari Area 2")
	end
end
